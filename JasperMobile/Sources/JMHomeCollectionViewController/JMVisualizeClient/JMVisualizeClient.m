...
- (void)setup {
    ...
    if (html) {
        NSURL *baseURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@/", self.resourceClient.serverProfile.serverUrl]];
...
- (void)runDashboard {
    if (!self.isActive) {
        ...
        credentials = [credentials stringByReplacingOccurrencesOfString:@"NAME" withString:self.resourceClient.serverProfile.username];
        credentials = [credentials stringByReplacingOccurrencesOfString:@"PASSWORD" withString:self.resourceClient.serverProfile.username];
