...
//    [self.bridge injectJSInitCode:script];
    ...
    self.report.script = script;

//    NSError *error;
//    NSString *htmlPath = [[NSBundle mainBundle] pathForResource:@"rest_report" ofType:@"html"];
//    NSString *html = [NSString stringWithContentsOfFile:htmlPath encoding:NSUTF8StringEncoding error:&error];

//    [self.report updateHTMLString:html
//                     baseURLSring:self.report.baseURLString];

//    [self.report updateScriptWithWidth:1000 height:1800];

//    [self.report updateHighchartContainerId];
