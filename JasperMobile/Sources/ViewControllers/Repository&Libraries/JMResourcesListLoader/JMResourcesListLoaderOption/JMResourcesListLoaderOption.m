...
- (instancetype)initWithType:(JMResourcesListLoaderOptionType)type title:(NSString *)title value:(id)value
{
    ...
    if (self) {
        ...
        if (type == JMResourcesListLoaderOptionType_Filter) {
            if (![value isKindOfClass:[NSArray class]]) {
                if ([[JMUtils supportedFormatsForReportSaving] indexOfObject:value] == NSNotFound) {
                    value = @[value, kJS_WS_TYPE_UNKNOW]; //We should send kJS_WS_TYPE_UNKNOW here according to http://jira.jaspersoft.com/browse/JRS-11049
                } else {
                    value = @[value];
                }
            }
