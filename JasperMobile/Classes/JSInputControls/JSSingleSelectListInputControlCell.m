...
- (id)initWithInputControlDescriptor:(JSInputControlDescriptor *)icDescriptor resourceDescriptor:(JSResourceDescriptor *)resourceDescriptor tableViewController:(UITableViewController *)tv {
	...
	if (self = [super initWithInputControlDescriptor:icDescriptor resourceDescriptor:resourceDescriptor tableViewController:tv]) {
        ...
        for (JSInputControlOption *option in self.icDescriptor.state.options) {
...
- (BOOL)isModified:(id)valuesToSet {
    return [self.selectedValue isEqualToString:valuesToSet];
}
...
- (void)requestFinished:(JSOperationResult *)result {
    ...
    if (result == nil || result.objects.count == 0) {
        ...
        for (JSResourceProperty *prop in [self.descriptor inputControlQueryData]) {
			JSListItem *item = [[JSListItem alloc] initWithName:prop.name andValue:prop.value selected:NO];
			[myItems addObject:item];
        }
...
- (void)adjustSelection {
	...
	if (newValue == nil && self.items.count > 0) {
        JSListItem *selectedItem = [self.items objectAtIndex:0];
