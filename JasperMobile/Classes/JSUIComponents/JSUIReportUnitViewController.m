...
- (void)viewWillAppear:(BOOL)animated {
    // Workaround... for HTML format there are not "good" idea to have next/prev
    // buttons on UIWebView. Also this cause a problem when we have a link instead
    // of file.
...
- (void)executeReport {
	...
	if ([self descriptor] != nil) {
        ...
        if ([self.reportClient.serverProfile.serverInfo versionAsInteger] >= [JSConstants sharedInstance].VERSION_CODE_EMERALD) {
            NSString *url = [self.reportClient generateReportUrl:self.descriptor.uriString reportParams:self.parameters page:0 format:self.format];
            if ([self.format isEqualToString:@"HTML"]) {
                ...
                [theConnection start];
                [JSUILoadingView showCancelableAllRequestsLoadingInView:self.view restClient:nil cancelBlock:^{
...
- (void)fileRequestFinished:(NSString *)path {
	...
	if ( [myFM isReadableFileAtPath:path] ) {
		...
		if ([self.format isEqualToString: @"PDF"]) {
            ...
            if (!pages) {
                pages = CGPDFDocumentGetNumberOfPages(myDocumentRef);
            }
    ...
    if (res.error != nil) {
		...
		{			
			...
			if ([[res.objects objectAtIndex:0] attachments].count == 0) {
                [[UIAlertView localizedAlert:nil message:@"error.reportissue.dialog.title" delegate:nil cancelButtonTitle:@"dialog.button.ok" otherButtonTitles:nil] show];
			} else {
				return;
			}
		} else {
            [[UIAlertView localizedAlert:nil message:@"error.reportempty.dialog.title" delegate:nil cancelButtonTitle:@"dialog.button.ok" otherButtonTitles:nil] show];
		}
	}
...
- (void)connection:(NSURLConnection *)connection didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    if ([challenge previousFailureCount] == 0) {
        NSURLCredential *newCredential = [NSURLCredential credentialWithUser:self.reportClient.serverProfile.username
                                                                    password:self.reportClient.serverProfile.password
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    self.tempDirectory = [paths objectAtIndex:0];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if (![fileManager fileExistsAtPath:self.tempDirectory]) {
        [fileManager createDirectoryAtPath:self.tempDirectory withIntermediateDirectories: YES attributes: nil error:NULL];
    }

    NSString *reportFile = [self.tempDirectory stringByAppendingPathComponent: [NSString stringWithFormat:@"%@%@", self.descriptor.name, self.format]];
