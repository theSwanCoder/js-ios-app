			...
			for (int i = 0; i < self.descriptor.childResourceDescriptors.count; ++i) {
				...
				if ([rd.wsType isEqualToString:constants.WS_TYPE_INPUT_CONTROL]) {
					...
					if ([isPropVisible isEqualToString:@"true"]) {	
						...
						JSInputControlCell *cell = [JSInputControlCell inputControlWithDescriptor:rd tableViewController: self dataSourceUri: reportUnitDsUri resourceClient:[JasperMobileAppDelegate sharedInstance].resourceClient];
...
- (void)updateTableContent {
    JSRESTResource *res = [JasperMobileAppDelegate sharedInstance].resourceClient;
	...
	if ([self descriptor] != nil) {
        ...
        [res resourceWithQueryData:descriptor.uriString datasourceUri:@"true" resourceParameters:nil delegate:self];
	}		
}
